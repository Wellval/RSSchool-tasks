...
export class GaragePage extends Page {
  ...
  constructor(rootElement: HTMLElement) {
    ...
    const pageNum = Number.parseInt(localStorage.getItem('page number')!, 10) || 1;
    ...
    this.resetBtn.element.addEventListener('click', async () => {
      ...
      this.isRacing = false;
      this.winner = null;
      for (const car of this.carRoutes) {
        car.reset();
      }

      for (const car of this.garage.cars) {
        car.stop();
      }
  ...
  async generateCars() {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      createRequests.push(this.garage.create(`${brand} ${name}`, color));
    }
